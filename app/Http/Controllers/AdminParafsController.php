...
use BaconQrCode\Writer;
	...
	class AdminParafsController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->form = [];
			$this->form[] = ['label'=>'Jenis','name'=>'jenis','type'=>'select','validation'=>'required|min:1|max:255','width'=>'col-sm-10','dataenum'=>'Surat;Invoice'];
			$this->form[] = ['label'=>'Kategori','name'=>'kategori','type'=>'select','validation'=>'required|min:1|max:255','width'=>'col-sm-10','dataenum'=>'Internal;Eksternal'];
			$this->form[] = ['label'=>'Tipe','name'=>'type','type'=>'select','validation'=>'required|min:1|max:255','width'=>'col-sm-10','dataenum'=>'Private;Publish'];
	        ...
	        */
            ...
            $this->addaction[] = ['label'=>'Generate Qr','url'=>CRUDBooster::mainpath('set-status/[id]'), 'color'=>'primary'];
	    ...
	    */
	    public function hook_before_add(&$postdata) {
            ...
            $uuid = \Str::uuid().$user_id.Carbon::now()->format('d');
            $cek = DB::table('parafs')->where('kategori', $postdata['kategori'])->count();
            $cek =(int)$cek+1;
            if($cek < 10){
                $cek = "0".$cek;
            }
            if($postdata['kategori'] === 'Eksternal'){
                $nomor = $cek."/B/".Carbon::now()->format('m/Y');
            }else{
                $nomor = $cek."/A/".Carbon::now()->format('m/Y');
	    ...
	    */
        ...
        public function getSetStatus($id) {
            ...
            if(!$data){
                ...
                CRUDBooster::redirect($_SERVER['HTTP_REFERER'],"Data not found","error");

            }

            if(!$data->image){
                DB::table('parafs')->where('id',$id)->update([
                    'image' => url('signature/'.$data->uuid)
                ]);
